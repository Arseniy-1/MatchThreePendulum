...
namespace Code.Infrastructure.Matcher
{
    public class CellMatcher : SerializedMonoBehaviour
    {
        ...
        private void OnBallHooked(Cell hookedCell, BallTypes ballType)
        {
            ...
            if (ballsToRemove.Count > 0)
            {
                ...
                MessageBrokerHolder.Game.Publish(new M_HasMatch(ballType));
            }
        ...
        private void CheckGameOver()
        {
            ...
            {
                for (int j = 0; j < _balls.GetLength(1); j++)
                {
                    if (_balls[i, j] == BallTypes.Unknown)
                        return;
                }
            }

            MessageBrokerHolder.Game.Publish(new M_GameOver());
        }
